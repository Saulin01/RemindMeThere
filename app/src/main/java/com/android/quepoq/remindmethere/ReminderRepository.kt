...
class ReminderRepository(private val context: Context) {
  ...
  fun add(reminder: Reminder, success: () -> Unit, failure: (error: String) -> Unit) {
    ...
    if (geofence != null && ContextCompat.checkSelfPermission(context, android.Manifest.permission.ACCESS_FINE_LOCATION)
  ...
  private fun buildGeofence(reminder: Reminder): Geofence? {
    ...
    if (latitude != null && longitude != null && radius != null) {
      return Geofence.Builder().setRequestId(reminder.id).setCircularRegion(
                      latitude,
                      longitude,
                      radius.toFloat()
